...
if __name__ == '__main__':
	...
	hdManager, _, _ = myUtil.getLVLMEmb(model, hdManager, lambda x, y: [0.0] * len(x), args.thres, layerIdxs,
									   ...
									   args.bs, args.embType, None, None, enableThink=args.et)
	# get the hd you prefer
	hdManager, _, _ = myUtil.getLVLMEmb(model, hdManager, lambda x, y: [-2] * len(x), [-3, -1], layerIdxs,
	...
	for i in range(args.maxIter):
		...
		if harmValPrompts is not None:
			...
			valCompletion, allRes = myUtil.GenAndEvalLVLM(model, processor, judgeF, harmValPrompts, args.trainL, args.bs, None, False, enableThink=args.et)
